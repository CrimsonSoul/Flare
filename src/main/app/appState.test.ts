...
import { loadConfigAsync, ensureDataFilesAsync } from '../dataUtils';
...
describe('appState', () => {
  ...
  describe('setupIpc', () => {
    it('wires IPC handlers and logger handlers', () => {
      state.mainWindow = { webContents: { getURL: vi.fn(() => 'http://localhost') } } as never;

      setupIpc();

      expect(setupIpcHandlers).toHaveBeenCalledWith(expect.any(Function), expect.any(Function));
      expect(setupLoggerHandlers).toHaveBeenCalled();

      const [getMainWindow] = vi.mocked(setupIpcHandlers).mock.calls[0] as [() => unknown];
